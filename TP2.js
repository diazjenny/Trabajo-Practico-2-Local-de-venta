...
const precioMaquina = (componentes) => {
    ...
    componentes.forEach(item => {
        const componente = precios.find(precio => precio.componente === item);
...
const cantidadVentasComponente = (componente) => {
    let cantidad = 0;
    ventas.forEach(item => {
        cantidad += item.componentes.filter(elemento => elemento === componente).length;
    });
    return cantidad;
}
...
const vendedoraDelMes = (mes, anio) => {
    ...
    vendedoras.forEach(item => {
        let suma = 0;
        const ventasMesVendedora = ventasMes.filter(venta => venta.nombreVendedora === item);
...
const ventasMes = (mes, anio) => {
    ...
    ventasMes.forEach(item => importe += precioMaquina(item.componentes));
    return importe;
}
...
const ventasVendedora = (nombre) => {
    ...
    ventasTotales.forEach(item => total += precioMaquina(item.componentes));
    return total;
}
...
const agregarSucursal = () => {
    ventas.forEach(item => item.sucursal = "Centro");
    return ventas;
}
...
const modificarVentas = () => {
    ...
    nuevasVentas.forEach(item => ventas.push(item));
    return ventas;
}
...
const renderPorMes = () => {
    ...
    for (i = 1; i <= 12; i++) {
        ...
        ventasTotalesMes.forEach(item => total += precioMaquina(item.componentes));
...
const renderPorSucursal = () => {
    ...
    sucursales.forEach(item => ventasPorSucursal.push({ sucursal: item, valor: ventasSucursal(item) }));
...
const mejorVendedora = () => {
    ...
    vendedoras.forEach(vendedora => {
        ...
        ventasMesVendedora.forEach(item => suma += precioMaquina(item.componentes));
        if (importeMejor < suma) {
            importeMejor = suma;
